...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(void) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(const int capacity) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
    //--- set capacity
...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(IEqualityComparer<TKey> *comparer) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(const int capacity, IEqualityComparer<TKey> *comparer) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
    if (capacity > 0) Initialize(capacity);
    //--- check equality comaprer
    if (CheckPointer(comparer) == POINTER_INVALID) {
...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(IMap<TKey, TValue> *map) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
    //--- use default equality comaprer
    m_comparer = new CDefaultEqualityComparer<TKey>();
    m_delete_comparer = true;
    ...
    if (CheckPointer(map) != POINTER_INVALID && map.Count() > 0) {
        ...
        for (int i = 0; i < map.Count(); i++) Add(keys[i], values[i]);
    }
}
//+------------------------------------------------------------------+
//| Initializes a new instance of the CHashMap<TKey,TValue> class    |
...
//+------------------------------------------------------------------+
template <typename TKey, typename TValue>
CHashMap::CHashMap(IMap<TKey, TValue> *map, IEqualityComparer<TKey> *comparer) : m_count(0), m_free_list(0), m_free_count(0), m_capacity(0)
{
    //--- check equality comaprer
    if (CheckPointer(comparer) == POINTER_INVALID) {
        //--- use default equality comaprer
...
{
    if (m_delete_comparer) delete m_comparer;
}
//+------------------------------------------------------------------+
...
{
    //--- check pair
    if (CheckPointer(pair) == POINTER_INVALID) return (false);
...
bool CHashMap::ContainsValue(TValue value)
{
    ...
    for (int i = 0; i < m_count; i++)
        if (m_entries[i].hash_code >= 0 && comparer_value.Equals(m_entries[i].value, value)) return (true);
    return (false);
}
//+------------------------------------------------------------------+
...
{
    //--- resize array
    if (dst_start + m_count > ArraySize(dst_array)) ArrayResize(dst_array, dst_start + m_count);
    //--- start copy
    int index = 0;
    for (int i = 0; i < ArraySize(m_entries); i++)
        if (m_entries[i].hash_code >= 0) {
            //--- check indexes
            if (dst_start + index >= ArraySize(dst_array) || index >= m_count) return (index);
...
int CHashMap::CopyTo(TKey &dst_keys[], TValue &dst_values[], const int dst_start = 0)
{
    ...
    //--- resize keys array
    if (dst_start + count > ArraySize(dst_keys)) ArrayResize(dst_keys, dst_start + count);
    //--- resize values array
    if (dst_start + count > ArraySize(dst_values)) ArrayResize(dst_values, MathMin(ArraySize(dst_keys), dst_start + count));
    //--- start copy
    int index = 0;
    for (int i = 0; i < ArraySize(m_entries); i++)
        if (m_entries[i].hash_code >= 0) {
            //--- check indexes
            if (dst_start + index >= ArraySize(dst_keys) || dst_start + index >= ArraySize(dst_values) || index >= count) return (index);
...
bool CHashMap::Remove(CKeyValuePair<TKey, TValue> *item)
{
    ...
    CDefaultEqualityComparer<TValue> comparer_value();
    //--- remove pair
    if (i >= 0 && comparer_value.Equals(m_entries[i].value, item.Value())) return Remove(item.Key());
    return (false);
}
//+------------------------------------------------------------------+
...
{
    //--- resize buckets
    if (ArrayResize(m_buckets, new_size) != new_size) return (false);
    ArrayFill(m_buckets, 0, new_size, -1);
    //--- resize entries
    if (ArrayResize(m_entries, new_size) != new_size) return (false);
    //--- restore buckets
    for (int i = 0; i < m_count; i++)
        if (m_entries[i].hash_code >= 0) {
...
int CHashMap::FindEntry(TKey key)
{
    if (m_capacity != NULL) {
        ...
        for (int i = m_buckets[hash_code % m_capacity]; i >= 0; i = m_entries[i].next)
            if (m_entries[i].hash_code == hash_code && m_comparer.Equals(m_entries[i].key, key)) return (i);
    }
    return (-1);
}
//+------------------------------------------------------------------+
