...
class JSONValue : public HashValue {
   ...
   public:
    ...
    static bool isString(JSONValue *val) { return val != NULL && val.isString(); }

    static bool isBool(JSONValue *val) { return val != NULL && val.isBool(); }

    static bool isNumber(JSONValue *val) { return val != NULL && val.isNumber(); }

    static bool isDouble(JSONValue *val) { return val != NULL && val.isNumber() && ((JSONNumber *)val).isDouble(); }

    static bool isLong(JSONValue *val) { return val != NULL && val.isNumber() && ((JSONNumber *)val).isLong(); }

    static bool isLongType(JSONValue *val) { return val != NULL && val.isNumber() && ((JSONNumber *)val).isLongType(); }

    static bool isInt(JSONValue *val) { return val != NULL && val.isNumber() && ((JSONNumber *)val).isInt(); }

    static bool isIntType(JSONValue *val) { return val != NULL && val.isNumber() && ((JSONNumber *)val).isIntType(); }

    static bool isArray(JSONValue *val) { return val != NULL && val.isArray(); }

    static bool isObject(JSONValue *val) { return val != NULL && val.isObject(); }

    static bool isNull(JSONValue *val) { return val != NULL && val.isNull(); }

    int get_char_position() { return _pos; }
...
class JSONString : public JSONValue {
   ...
   public:
    ...
    JSONString(int pos = 0, int line = 0, int column = 0) : JSONValue(pos, line, column) { setType(JSON_STRING); }
        ...
        for (i = 0; i < slen; i++) {
            ...
            for (i = 0; i < slen; i++) {
                ...
                if (uc == 0x22 /* " */ || uc == 0x5C /* \ */) {
                    ...
                    StringSetCharacter(str, clen++, 0x5C /* \ */);
                    if (0x08 == uc) { /* backspace       U+0008 */
                        StringSetCharacter(str, clen++, 'b');
                    }
                    else if (0x09 == uc) { /* tab             U+0009 */
                        StringSetCharacter(str, clen++, 't');
                    }
                    else if (0x0A == uc) { /* line feed       U+000A */
                        StringSetCharacter(str, clen++, 'n');
                    }
                    else if (0x0C == uc) { /* form feed       U+000C */
                        StringSetCharacter(str, clen++, 'f');
                    }
                    else if (0x0D == uc) { /* carriage return U+000D */
    ...
    class JSONBool : public JSONValue {
       ...
       public:
        ...
        JSONBool(int pos = 0, int line = 0, int column = 0) : JSONValue(pos, line, column) { setType(JSON_BOOL); }
    ...
    class JSONNumber : public JSONValue {
       ...
       public:
        ...
        bool isIntType()
        { // Is int type (e.g. parsed from 5 but not from 5.0) ?
            return false == type_is_double && _long == ((long)((int)_long));
        }
        bool isInt()
        { // Is int type or convertible lossless to int ?
            return isIntType() || (true == type_is_double && _dbl == ((double)((int)_dbl)));
        }
        bool isLongType()
        { // Is long type (e.g. parsed from 5 but not from 5.0) ?
            return false == type_is_double;
        }
        bool isLong()
        { // Is long type or convertible lossless to long ?
            return false == type_is_double || (true == type_is_double && _dbl == ((double)((long)_dbl)));
        }
        bool isDouble()
        { // Is double type or convertible lossless to double ?
        ...
        bool isDoubleType()
        { // Is double type (e.g. parsed from 5.0 but not from 5) ?
        ...
        string toString()
        {
            if (type_is_double) {
                ...
                if (StringFind(str, ".") >= 0 || StringFind(str, "E") >= 0 || StringFind(str, "e") >= 0) {
                    if (StringLen(str) >= 2 && (StringGetCharacter(str, 0) == '+' || StringGetCharacter(str, 0) == '-') &&
                        StringGetCharacter(str, 1) == '.') {
                        str = StringSubstr(str, 0, 1) + "0" +
                              StringSubstr(str, 1); // JSON does not allow the notation e.g. .5 instead of 0.5 .
                    }
    ...
    class JSONNull : public JSONValue {
       public:
        JSONNull(int pos = 0, int line = 0, int column = 0) : JSONValue(pos, line, column) { setType(JSON_NULL); }
        ~JSONNull() {}
    ...
    class JSONObject : public JSONValue {
       ...
       public:
        ...
        bool isString(string key) { return isString(getValue(key)); }
        bool isNumber(string key) { return isNumber(getValue(key)); }
        bool isDouble(string key) { return isDouble(getValue(key)); }
        bool isInt(string key) { return isInt(getValue(key)); }
        bool isIntType(string key) { return isIntType(getValue(key)); }
        bool isLong(string key) { return isLong(getValue(key)); }
        bool isLongType(string key) { return isLongType(getValue(key)); }
        bool isBool(string key) { return isBool(getValue(key)); }
        bool isArray(string key) { return isArray(getValue(key)); }
        bool isObject(string key) { return isObject(getValue(key)); }
        bool isNull(string key) { return isNull(getValue(key)); }

        /// Lookup key and get associated string value, return false if failure.
        bool getString(string key, string &out) { return getString(getValue(key), out); }
        ...
        bool getDouble(string key, double &out) { return getDouble(getValue(key), out); }
        ...
        bool getLongType(string key, long &out) { return getLongType(getValue(key), out); }
        ...
        bool getIntType(string key, int &out) { return getIntType(getValue(key), out); }
    ...
    class JSONArray : public JSONValue {
       ...
       public:
        ...
        bool isString(int index) { return isString(getValue(index)); }
        bool isNumber(int index) { return isNumber(getValue(index)); }
        bool isDouble(int index) { return isDouble(getValue(index)); }
        bool isInt(int index) { return isInt(getValue(index)); }
        bool isIntType(int index) { return isIntType(getValue(index)); }
        bool isLong(int index) { return isLong(getValue(index)); }
        bool isLongType(int index) { return isLongType(getValue(index)); }
        bool isBool(int index) { return isBool(getValue(index)); }
        bool isArray(int index) { return isArray(getValue(index)); }
        bool isObject(int index) { return isObject(getValue(index)); }
        bool isNull(int index) { return isNull(getValue(index)); }

        /// Lookup JSONString by array index. NULL if not present. Cast failure if not an Object.
        bool getString(int index, string &out) { return getString(getValue(index), out); }
        ...
        bool getDouble(int index, double &out) { return getDouble(getValue(index), out); }
        /// Lookup JSONNumber by array index. NULL if not present. Cast failure if not an Object.
        bool getLong(int index, long &out) { return getLong(getValue(index), out); }
        bool getLongType(int index, long &out) { return getLongType(getValue(index), out); }
        /// Lookup JSONNumber by array index. NULL if not present. Cast failure if not an Object.
        bool getInt(int index, int &out) { return getInt(getValue(index), out); }
        bool getIntType(int index, int &out) { return getIntType(getValue(index), out); }
    ...
    ///       },"+ " \"phoneNumber\": [ { \"type\": \"home\", \"number\": \"212 555-1234\" }, { \"type\": \"fax\", \"number\": \"646
    ...
    class JSONParser {
       private:
        ...
        bool isDigit0to9(ushort c) { return (c >= '0' && c <= '9'); }
        ...
        {
            if (_pos >= _len) {
                setError(1, "expected " + ShortToString(c) + " (" + (string)c + ")" + " got end of file");
                return false;
            }
            else if (c == _in[_pos]) {
                _pos++;
            ...
            if (expect('"')) {
                while (true) {
                    ...
                    for (; end < _len && (c = _in[end]) != '"' && c != '\\'; end++) {
                        ...
                        if (c < 0x20) {
                            setError(
                                4, "Control characters not allowed in JSON strings. Found character with decimal ASCII code " + (string)c +
                                       " ."
                            );
                            _pos = end;
                            break;
        ...
        JSONValue *parseValue()
        {
            ...
            if (_in[_pos] == '[') {
                ...
                if (isDouble) {
                    ...
                    ret = (JSONValue *)new JSONNumber(
                        d, _begin_object_pos, _begin_object_line, _begin_object_column
                    ); // Create a Number as double only
                }
                else {
                    long l = sign * StringToInteger(s);
                    ret = (JSONValue *)new JSONNumber(
                        l, _begin_object_pos, _begin_object_line, _begin_object_column
                    ); // Create a Number as a long
                }
                _column += i - _pos;
                _pos = i;
            }
       ...
       public:
        ...
        JSONValue *parse(string s ///< Serialized JSON text
        )
        {
            int inLen;
            JSONValue *ret = NULL;
    ...
    class JSONIterator {
       ...
       public:
        ...
        bool isValid() { return _l.isValid(); }

        // Check if current item is valid and so the function
        // val() will not return NULL.
        // Deprecated since the name hasNext is misleading
        bool hasNext() { return _l.isValid(); }
