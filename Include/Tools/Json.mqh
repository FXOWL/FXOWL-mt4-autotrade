    ...
    string toString()
    {
        ...
        if (_hash != NULL) {
            ...
            for (l = new HashLoop(_hash); l.hasNext(); l.next()) {
                ...
                s = StringConcatenate(s, (++n == 1 ? "" : ","), "\"", l.key(), "\" : ", v.toString());
            }
            delete l;
        }
        s = s + "}";
...
class JSONParser {
   ...
   public:
    ...
    bool isDigit(ushort c) { return (c >= '0' && c <= '9') || c == '+' || c == '-'; }

    bool isDoubleDigit(ushort c) { return (c >= '0' && c <= '9') || c == '+' || c == '-' || c == '.' || c == 'e' || c == 'E'; }
    ...
    bool expect(ushort c)
    {
        ...
        if (c == _in[_pos]) {
            ...
            setError(
                1, StringConcatenate("expected ", ShortToString(c), "(", c, ")", " got ", ShortToString(_in[_pos]), "(", _in[_pos], ")")
            );
        }
        return ret;
    }
    ...
    string parseString()
    {
        ...
        if (expect('"')) {
            while (true) {
                ...
                if (end >= _len) {
                    setError(
                        2, "missing quote: end" + (string)end + ":len" + (string)_len + ":" + ShortToString(_in[_pos]) + ":" +
    ...
    JSONValue *parseValue()
    {
        ...
        if (_in[_pos] == '[') {
            ...
            if (isDoubleDigit(_in[i])) {
                ...
                string str = StringSubstr(_instr, _pos, i - _pos);
                double d = sign * StringToDouble(str);
...
{
    string s =
        "{ \"firstName\": \"John\", \"lastName\": \"Smith\", \"age\": 25, \"address\": { \"streetAddress\": \"21 2nd Street\", \"city\": "
        "\"New York\", \"state\": \"NY\", \"postalCode\": \"10021\" }, \"phoneNumber\": [ { \"type\": \"home\", \"number\": \"212 "
        "555-1234\" }, { \"type\": \"fax\", \"number\": \"646 555-4567\" } ], \"gender\":{ \"type\":\"male\" }  }";
