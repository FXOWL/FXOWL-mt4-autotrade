...
///      // 'true' means when the hash will adopt the values and delete them when they are removed from the hash or when the hash is
///      deleted.
///
///      h.hPut("a",a);
///      h.hPut("b",b);
...
class Hash : public HashValue {
   private:
    ...
    /// To store an object against a string(key) - get the string hashcode, then insert pair (key,val) into the linked list for that
    /// hashcode. To fetch an object against a string(key) - get the string hashcode, get linked-list at that hashcode index, then search
