...
//+------------------------------------------------------------------+
template <typename T>
CHashSet::CHashSet(IEqualityComparer<T> *comparer) : m_count(0), m_last_index(0), m_free_list(-1)
{
...
//+------------------------------------------------------------------+
template <typename T>
CHashSet::CHashSet(ICollection<T> *collection) : m_count(0), m_last_index(0), m_free_list(-1)
{
    //--- use default equality comaprer
    m_comparer = new CDefaultEqualityComparer<T>();
    m_delete_comparer = true;
    //--- check collection
    if (CheckPointer(collection) == POINTER_INVALID) return;
    //--- set capacity for elements of the collection
    int count = collection.Count();
    Initialize(count);
    //--- add element from collection to the set
    this.UnionWith(collection);
    if ((m_count == 0 && ArraySize(m_slots) > 3) || (m_count > 0 && ArraySize(m_slots) / m_count > 3)) TrimExcess();
}
//+------------------------------------------------------------------+
//| Initializes a new instance of the CHashSet<T> class that uses the|
...
//+------------------------------------------------------------------+
template <typename T>
CHashSet::CHashSet(ICollection<T> *collection, IEqualityComparer<T> *comparer) : m_count(0), m_last_index(0), m_free_list(-1)
{
    //--- check equality comaprer
    if (CheckPointer(comparer) == POINTER_INVALID) {
        //--- use default equality comaprer
...
//+------------------------------------------------------------------+
template <typename T>
CHashSet::CHashSet(T &array[], IEqualityComparer<T> *comparer) : m_count(0), m_last_index(0), m_free_list(-1)
{
    //--- check equality comaprer
    if (CheckPointer(comparer) == POINTER_INVALID) {
        //--- use default equality comaprer
...
bool CHashSet::Contains(T item)
{
    ...
    if (ArraySize(m_buckets) != 0) {
        ...
        for (int i = m_buckets[hash_code % ArraySize(m_buckets)] - 1; i >= 0; i = m_slots[i].next)
            if (m_slots[i].hash_code == hash_code && m_comparer.Equals(m_slots[i].value, item)) return (true);
    }
    return (false);
}
//+------------------------------------------------------------------+
...
void CHashSet::ExceptWith(ICollection<T> *collection)
{
    ...
    for (int i = 0; i < ArraySize(array); i++) Remove(array[i]);
}
//+------------------------------------------------------------------+
...
void CHashSet::IntersectWith(T &array[])
{
    ...
    for (int i = 0; i < m_last_index; i++) {
        if (m_slots[i].hash_code >= 0) {
            T item = m_slots[i].value;
            if (!set.Contains(item)) Remove(item);
        }
    }
}
//+------------------------------------------------------------------+
...
{
    for (int i = 0; i < ArraySize(array); i++) AddIfNotPresent(array[i]);
}
//+------------------------------------------------------------------+
...
{
    //--- check collection
    if (CheckPointer(collection) == POINTER_INVALID) return (false);
    //--- the empty set is a proper subset of anything but the empty set
    if (m_count == 0) return (collection.Count() > 0);
    //--- check collection is set
    CHashSet<T> *ptr_set = dynamic_cast<CHashSet<T> *>(collection);
    if (CheckPointer(ptr_set) != POINTER_INVALID) {
...
{
    //--- the empty set is a proper subset of anything but the empty set
    if (m_count == 0) return (ArraySize(array) > 0);
...
{
    //--- check collection
    if (CheckPointer(collection) == POINTER_INVALID) return (m_count > 0);
    //--- the empty set is a proper subset of anything but the empty set
...
bool CHashSet::SetEquals(ICollection<T> *collection)
{
    ...
    if (collection == GetPointer(this)) return (true);
    //--- get array from collection
    T array[];
    collection.CopyTo(array);
...
{
    //--- check size
    if (ArraySize(array) != m_count) return (false);
    //--- check current set is equal specified array
    for (int i = 0; i < ArraySize(array); i++)
...
void CHashSet::SetCapacity(const int new_size, bool new_hash_codes)
{
    ...
    if (new_hash_codes)
        for (int i = 0; i < m_last_index; i++)
            if (m_slots[i].hash_code != -1) m_slots[i].hash_code = m_comparer.HashCode(m_slots[i].value) & 0x7FFFFFFF;
    //--- resize buckets
    ArrayResize(m_buckets, new_size);
...
bool CHashSet::AddIfNotPresent(T value)
{
    ...
    if (ArraySize(m_buckets) == 0) Initialize(0);
    //--- get hash code and bucket for value
    int hash_code = m_comparer.HashCode(value) & 0x7FFFFFFF;
    int bucket = hash_code % ArraySize(m_buckets);
    //--- check value already in the set
    for (int i = m_buckets[hash_code % ArraySize(m_buckets)] - 1; i >= 0; i = m_slots[i].next)
        if (m_slots[i].hash_code == hash_code && m_comparer.Equals(m_slots[i].value, value)) return (false);
...
bool CHashSet::InternalIsSubsetOf(CHashSet<T> *set)
{
    ...
    if (m_count > set.m_count) return (false);
...
bool CHashSet::InternalIsSupersetOf(CHashSet<T> *set)
{
    ...
    if (set.m_count > m_count) return (false);
