...
static const double VERSION = 2.0;
...
input string I_CURRENCY_PAIRS =
    "USDJPY,EURUSD,GBPUSD,AUDUSD,USDCAD,USDCHF,EURGBP,EURJPY,AUDJPY,NZDJPY,GBPJPY,NZDUSD,XAUUSD"; // 表示通貨ペア(カンマ区切り)
...
input DISPLAY_CORNER I_CORNER = CORNER_LEFT_UPPER; // トレンドテーブル表示位置
input color I_TIMEFRAME_COLOR = clrRed; // 時間軸の文字色
input color I_CURRENCYPAIR_COLOR = clrGold; // 通貨ペアの文字色
input color I_STAGE1_COLOR = clrOrange; // Stage1のシンボルの色
input color I_STAGE2_COLOR = clrYellow; // Stage2のシンボルの色
input color I_STAGE3_COLOR = clrSkyBlue; // Stage3のシンボルの色
input color I_STAGE4_COLOR = clrTeal; // Stage4のシンボルの色
input color I_STAGE5_COLOR = clrSkyBlue; // Stage5のシンボルの色
input color I_STAGE6_COLOR = clrYellow; // Stage6のシンボルの色
input color I_ARROW_NORMAL_COLOR = clrGray;
...
// シンボル表示
static const string FONT_DECORATION_SYMBOL = "Wingdings 2"; // シンボルの表示で使用する装飾記号
static const int SYMBOLCODE_STAGE1 = 117;
static const int SYMBOLCODE_STAGE2 = 118;
static const int SYMBOLCODE_STAGE3 = 119;
static const int SYMBOLCODE_STAGE4 = 120;
static const int SYMBOLCODE_STAGE5 = 121;
static const int SYMBOLCODE_STAGE6 = 122;
static const int SYMBOLCODE_NO_SIGNAL = 218;
static const int SYMBOL_SIZE = 12;
...
)
{
    ...
    for (int currency_no = 0; currency_no < ArraySize(currency_pairs); currency_no++) {
        ...
        for (int period_num = 0; period_num < ArraySize(periodStrings); period_num++) {
            ...
            MovingAverage3Line *signal = createMovingAverage3Line(currency_pair, timeframe, 0);

            string unique_key = CreateCurrencySignalMapKey(currency_pair, period_num);
            curency_signal_map.Add(unique_key, signal);

            if (signal.IsStage1()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE1), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE1_COLOR);
            }
            else if (signal.IsStage2()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE2), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE2_COLOR);
            }
            else if (signal.IsStage3()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE3), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE3_COLOR);
            }
            else if (signal.IsStage4()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE4), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE4_COLOR);
            }
            else if (signal.IsStage5()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE5), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE5_COLOR);
            }
            else if (signal.IsStage6()) {
                ObjectSetText(unique_key, CharToStr(SYMBOLCODE_STAGE6), SYMBOL_SIZE, FONT_DECORATION_SYMBOL, I_STAGE6_COLOR);
            }
            else {
